...
def align_and_load_brainiac(pmid_list, text_embeddings_array):
    """
    Checks for the physical existence of .npy files and returns paired
    brain features and text embeddings.
    """
    valid_indices = []
    brain_features = []
...
# Replacing DiFuMo/Gaussian with BrainIAC while keeping text aligned
...
dropout = 0.6
num_epochs = 50
latent_size = 512  # Shared latent space dimension (k) [cite: 155, 167]
...
# Initialize test dataset with BrainIAC features
...
for fold, (train_index, val_index) in enumerate(
    ...
    # UPDATED MODEL: Map 768 BrainIAC -> 512 Latent Space
    model = ClipModel(
    ...
    #     model, train_loader, val_loader, optimizer, None, criterion,
    ...
    clip_model, clip_train_loss, clip_val_loss, callback_outputs = train(
        ...
        val_loader,
        optimizer,
        criterion,  # 5th argument: the loss function
        None,  # 6th argument: the scheduler (can be None)
        num_epochs,
        device,
        verbose=True,
    ...
    for loader_name, loader, weights_path in [
        ...
        metrics[loader_name]["recall@10"].append(
            recall_fn(similarity, np.eye(len(similarity)), n_first=10)
        )  # [cite: 358]
        metrics[loader_name]["recall@100"].append(
            recall_fn(similarity, np.eye(len(similarity)), n_first=100)
        )  # [cite: 358]
        metrics[loader_name]["mix_match"].append(
            100 * mix_match(similarity)
        )  # [cite: 360, 361]
